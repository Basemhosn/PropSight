  ...
  return (
    <div style={{flex:1,overflowY:'auto',background:'#060E1A',fontFamily:'system-ui',padding:'28px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
        <div>
          <h1 style={{margin:0,fontSize:22,fontWeight:700,color:'#F1F5F9',marginBottom:4}}>Market <span style={{color:'#38BDF8'}}>Pulse</span></h1>
      ...
      {loading ? (
        <div style={{textAlign:'center',padding:80,display:'flex',flexDirection:'column',alignItems:'center',gap:16}}>
          ...
          <div style={{fontSize:14,color:'#64748B'}}>Generating AI market analysis...</div>
        ...
        {summary && (()=>{
          ...
          return <>
            <div style={{background:`linear-gradient(135deg,${ss.bg},rgba(6,14,26,0.5))`,border:`1px solid ${ss.border}`,borderRadius:16,padding:24,marginBottom:24}}>
              ...
              <div style={{fontSize:20,fontWeight:700,color:'#F1F5F9',marginBottom:10,lineHeight:1.3}}>{summary.headline}</div>
              <div style={{fontSize:13,color:'#94A3B8',lineHeight:1.6,marginBottom:12}}>{summary.weekSummary}</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:24}}>
              {summary.keyInsights?.map((ins,i)=>(
                <div key={i} style={{background:'#0D1929',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:18}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                    <span style={{fontSize:18,color:TC[ins.trend],fontWeight:700}}>{TI[ins.trend]}</span>
                    <span style={{fontSize:12,fontWeight:700,color:'#F1F5F9'}}>{ins.title}</span>
                  </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:12,marginBottom:24}}>
          {[['Total Transactions',fmtNum(allSlice.count||0),'#F1F5F9'],['Market Value',fmtAED(allSlice.total||0,true),'#38BDF8'],['Avg Deal',fmtAED(allSlice.count?Math.round(allSlice.total/allSlice.count):0,true),'#F1F5F9'],['Price/sqft','AED '+fmtNum(priceTrend[priceTrend.length-1]?.ppsqft||0),'#22C55E']].map(([l,v,c],i)=>(
            <div key={i} style={{background:'#0D1929',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:'16px 18px'}}>
              <div style={{fontSize:11,color:'#64748B',marginBottom:8,textTransform:'uppercase',letterSpacing:'0.05em'}}>{l}</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:24}}>
          <div style={{background:'#0D1929',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:20}}>
            <div style={{fontSize:13,fontWeight:600,color:'#F1F5F9',marginBottom:14}}>Monthly Volume</div>
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={monthly} margin={{top:4,right:4,left:0,bottom:0}}>
                ...
                <YAxis hide/>
                <Tooltip contentStyle={{background:'#0A1628',border:'1px solid rgba(59,130,246,0.2)',borderRadius:8,color:'#F1F5F9',fontSize:11}} formatter={v=>[fmtNum(v),'Transactions']}/>
                <Area type="monotone" dataKey="count" stroke="#38BDF8" strokeWidth={2.5} fill="url(#mpg1)"/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div style={{background:'#0D1929',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:20}}>
            <div style={{fontSize:13,fontWeight:600,color:'#F1F5F9',marginBottom:14}}>Price / sqft Trend</div>
            <ResponsiveContainer width="100%" height={160}>
        ...
        <div style={{background:'#0D1929',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,overflow:'hidden',marginBottom:24}}>
          <div style={{padding:'16px 20px',borderBottom:'1px solid rgba(255,255,255,0.06)'}}><div style={{fontSize:14,fontWeight:600,color:'#F1F5F9'}}>🔥 Top Areas This Week</div></div>
          {topAreas.map((area,i)=>(
        ...
        {summary?.outlook && (
          <div style={{background:'linear-gradient(135deg,rgba(29,78,216,0.1),rgba(56,189,248,0.05))',border:'1px solid rgba(59,130,246,0.2)',borderRadius:14,padding:20,textAlign:'center'}}>
            ...
            <div style={{fontSize:15,color:'#F1F5F9',lineHeight:1.6,fontStyle:'italic'}}>"{summary.outlook}"</div>
          </div>
        )}
      </>}
    </div>
