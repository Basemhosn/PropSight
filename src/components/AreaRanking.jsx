...
export default function AreaRanking({ rows, areas }) {
  ...
  const getColor = (a, i) => {
    if (metric === "growth") {
      ...
      return a.growth >= 0 ? "#1D9E75" : "#E24B4A";
    }
    const blues = ["#042C53","#0C447C","#185FA5","#378ADD","#85B7EB","#B5D4F4"];
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #E8ECF2", borderRadius:12, padding:"1.25rem" }}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:"1rem", flexWrap:"wrap", gap:8 }}>
        <div>
          <div style={{ fontSize:13, fontWeight:600, color:"#0A1628" }}>Top performing areas</div>
        ...
        <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
          {METRICS.map(m => (
            <button key={m.key} onClick={() => setMetric(m.key)} style={{
              ...
              background: metric===m.key ? "#EDF4FC" : "#fff",
              color: metric===m.key ? "#185FA5" : "#7A8499",
            }}>{m.label}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:4, marginBottom:"1rem", flexWrap:"wrap" }}>
        {propTypes.map(p => (
          <button key={p} onClick={() => setPropType(p)} style={{
            ...
            background: propType===p ? "#0A1628" : "#fff",
            color: propType===p ? "#fff" : "#9AA0AE",
        ...
        {ranked.map((a, i) => {
          ...
          return (
            <div key={a.area} style={{ display:"flex", alignItems:"center", gap:10 }}>
              {/* Rank */}
              <div style={{ width:22, fontSize:11, fontWeight:700, color: i < 3 ? "#185FA5" : "#C5CAD6", textAlign:"right", flexShrink:0 }}>
                {i+1}
              </div>
              {/* Area name */}
              <div style={{ width:148, fontSize:12, color:"#0A1628", flexShrink:0, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                {a.area}
              </div>
              {/* Bar */}
              <div style={{ flex:1, height:8, background:"#F4F6FA", borderRadius:4, overflow:"hidden" }}>
                <div style={{ width:`${barW}%`, height:"100%", background:getColor(a,i), borderRadius:4, transition:"width 0.3s" }} />
              </div>
              {/* Value */}
              <div style={{ width:90, fontSize:12, fontWeight:600, color: metric==="growth" && a.growth !== null ? (a.growth>=0 ? "#0F6E56":"#A32D2D") : "#0A1628", textAlign:"right", flexShrink:0 }}>
                {fmtVal(a)}
              </div>
            </div>
          );
