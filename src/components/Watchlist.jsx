  ...
  if (!isLite) return (
    <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:'#060E1A',fontFamily:'system-ui',padding:24}}>
      <div style={{textAlign:'center',maxWidth:380}}>
        ...
        <div style={{fontSize:22,fontWeight:700,color:'#F1F5F9',marginBottom:8}}>Watchlist</div>
        <div style={{fontSize:14,color:'#64748B',marginBottom:24,lineHeight:1.6}}>Save areas and projects you are tracking. Get notified when prices move.</div>
  ...
  return (
    <div style={{flex:1,overflowY:'auto',background:'#060E1A',fontFamily:'system-ui',padding:'24px 28px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
        <div>
          <h1 style={{margin:0,fontSize:22,fontWeight:700,color:'#F1F5F9',marginBottom:4}}>Watchlist</h1>
      ...
      : items.length === 0 ? (
        <div style={{textAlign:'center',padding:60}}>
          <div style={{fontSize:48,marginBottom:16}}>🔖</div>
          <div style={{fontSize:18,fontWeight:600,color:'#F1F5F9',marginBottom:8}}>Nothing saved yet</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:16}}>
          {items.map(item => {
            ...
            return (
              <div key={item.id} style={{background:'#0D1929',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:20}}>
                ...
                {item.type==='area' && currentPpsqm>0 && (
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:10}}>
                    <div style={{background:'rgba(59,130,246,0.06)',borderRadius:8,padding:10}}>
                      ...
                      <div style={{fontSize:13,fontWeight:700,color:'#F1F5F9'}}>AED {fmtNum(Math.round(currentPpsqm/10.764))}</div>
                    </div>
                    <div style={{background:'rgba(59,130,246,0.06)',borderRadius:8,padding:10}}>
                      <div style={{fontSize:9,color:'#475569',marginBottom:3}}>AVG DEAL</div>
                      <div style={{fontSize:13,fontWeight:700,color:'#F1F5F9'}}>{fmtAED(currentAvg,true)}</div>
                    </div>
                  </div>
                )}
                {change!==0 && <div style={{fontSize:12,fontWeight:600,color:change>0?'#22C55E':'#F87171'}}>{change>0?'↑':'↓'} {Math.abs(change)}% since added</div>}
                <div style={{fontSize:10,color:'#1E3A5F',marginTop:8}}>Added {new Date(item.created_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</div>
              </div>
            );
          })}
        </div>
      ...
      {showAdd && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
          <div style={{background:'#0D1929',border:'1px solid rgba(59,130,246,0.2)',borderRadius:16,padding:28,width:'100%',maxWidth:440,fontFamily:'system-ui'}}>
            <div style={{fontSize:18,fontWeight:700,color:'#F1F5F9',marginBottom:20}}>Add to Watchlist</div>
            <div style={{display:'flex',gap:8,marginBottom:16,background:'#070E1B',borderRadius:10,padding:4}}>
              {[['area','Area'],['project','Project']].map(([t,l])=>(
                <button key={t} onClick={()=>{setAddType(t);setAddValue('');}} style={{flex:1,padding:'8px',borderRadius:8,border:'none',cursor:'pointer',fontSize:13,fontFamily:'system-ui',fontWeight:addType===t?600:400,background:addType===t?'linear-gradient(135deg,#1D4ED8,#38BDF8)':'transparent',color:addType===t?'#fff':'#64748B'}}>{l}</button>
              ))}
            </div>
            <div style={{marginBottom:20}}>
              <label style={{fontSize:12,color:'#64748B',marginBottom:5,display:'block',fontWeight:500}}>Select {addType==='area'?'Area':'Project'}</label>
              <select value={addValue} onChange={e=>setAddValue(e.target.value)} style={{width:'100%',padding:'10px 12px',borderRadius:8,background:'#070E1B',border:'1px solid rgba(59,130,246,0.15)',color:'#F1F5F9',fontSize:13,outline:'none',fontFamily:'system-ui',cursor:'pointer'}}>
