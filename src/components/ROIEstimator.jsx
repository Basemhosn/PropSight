...
export default function ROIEstimator({ rows, areas }) {
  ...
  const card = (label, value, sub, color="#0A1628") => (
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #E8ECF2", borderRadius:12, padding:"1.25rem" }}>
      <div style={{ marginBottom:"1rem" }}>
        <div style={{ fontSize:13, fontWeight:600, color:"#0A1628" }}>ROI & yield estimator</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:"1.25rem" }}>
        ...
        <div style={{ gridColumn:"1/-1" }}>
          ...
          <select value={selectedArea} onChange={e => setSelectedArea(e.target.value)}
            style={{ width:"100%", fontSize:12, padding:"7px 10px", borderRadius:8, border:"1px solid #E8ECF2", background:"#F8FAFC", color:"#0A1628" }}>
        ...
        <div>
          ...
          <input value={purchasePrice}
            ...
            style={{ width:"100%", fontSize:12, padding:"7px 10px", borderRadius:8, border:"1px solid #E8ECF2", background:"#F8FAFC", color:"#0A1628", boxSizing:"border-box" }}
          />
        ...
        <>
          <div style={{ borderTop:"1px solid #F0F2F6", paddingTop:"1rem", marginBottom:"1rem" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {card("Gross yield", grossYield.toFixed(2)+"%", fmtAED(rent,true)+" / yr", "#185FA5")}
              {card("Net yield", netYield.toFixed(2)+"%", "After service charges", netYield >= 6 ? "#0F6E56" : "#BA7517")}
            </div>
          </div>
          ...
          <div style={{ borderTop:"1px solid #F0F2F6", paddingTop:"1rem" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              {card("Future value", fmtAED(futureValue,true), `+${fmtAED(capitalGain,true)} gain`, "#1D9E75")}
              {card("Total return", fmtAED(totalReturn,true), `${totalROI.toFixed(1)}% total ROI`, "#534AB7")}
              {card("Annual ROI", annualROI.toFixed(2)+"%", "Capital + rental", "#185FA5")}
