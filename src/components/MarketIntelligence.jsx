...
export default function MarketIntelligence({ areaData, core }) {
  ...
  const kpis = [
    ...
    { label:'Areas Tracked',     value:`${allAreas.length}+`,   sub:'Dubai-wide',       subColor:'#64748B' },
  ...
  return (
    <div style={{ flex:1, overflowY:'auto', background:'#060E1A', fontFamily:'system-ui', padding:'28px' }}>
      ...
      <div style={{ display:'flex', gap:12, marginBottom:24 }}>
        {kpis.map((k,i)=>(
          <div key={i} style={{ flex:1, background:'#0D1929', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:'16px 18px' }}>
            <div style={{ fontSize:11, color:'#64748B', marginBottom:8, textTransform:'uppercase', letterSpacing:'0.05em' }}>{k.label}</div>
      ...
      {/* Price trend chart */}
      <div style={{ background:'#0D1929', border:'1px solid rgba(255,255,255,0.06)', borderRadius:14, padding:20, marginBottom:20 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
          <div>
            <div style={{ fontSize:14, fontWeight:600, color:'#F1F5F9' }}>Price Trend (AED/sqft)</div>
      ...
      <div style={{ marginBottom:20 }}>
          ...
          <div style={{ background:'linear-gradient(135deg,#061A10,#0A2818,#051510)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:14, padding:24, marginBottom:12, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ flex:1 }}>
              ...
              <div style={{ display:'flex', gap:40 }}>
                {[
                  {l:'Price / sqft', v:`AED ${fmtNum(topArea.ppsqft)}`, c:'#F1F5F9'},
                  {l:'YoY Growth',   v:`${topArea.yoy>0?'+':''}${topArea.yoy}%`, c:'#22C55E'},
                  {l:'Rental Yield', v:`${topArea.rentalYield}%`, c:'#22C55E'},
                  {l:'Avg Deal',     v:fmtAED(topArea.avg,true), c:'#94A3B8'},
                ].map((item,i)=>(
                  <div key={i}>
                    <div style={{ fontSize:11, color:'#64748B', marginBottom:4 }}>{item.l}</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:10, marginBottom:20 }}>
          {secondTier.map((area,i)=>(
            <div key={area.key} onClick={()=>setActiveTab(area.key)} style={{ background:'#0D1929', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:16, cursor:'pointer', transition:'all 0.15s' }}
            ...
            >
              ...
              <div style={{ fontSize:13, fontWeight:700, color:'#F1F5F9', marginBottom:2 }}>{area.name.length>20?area.name.split(' ').slice(0,3).join(' '):area.name}</div>
              <div style={{ fontSize:11, color:'#475569', marginBottom:10 }}>{fmtNum(area.count)} transactions</div>
              <div style={{ fontSize:10, color:'#64748B', marginBottom:3 }}>PRICE / SQFT</div>
