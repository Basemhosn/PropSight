...
const CustomTooltip = ({ active, payload, label, metric }) => {
  ...
  return (
    <div style={{ background: "#0A1628", color: "#fff", borderRadius: 8, padding: "10px 14px", fontSize: 12, minWidth: 200 }}>
      ...
      {payload.map(p => {
        ...
        return (
          <div key={p.name} style={{ color: p.color, marginBottom: 3 }}>
            {p.name}: <span style={{ color: "#fff", fontWeight: 600 }}>{val}</span>
          </div>
        );
      })}
  ...
  if (years.length < 2) {
    return (
      <div style={{ background: "#fff", border: "1px solid #E8ECF2", borderRadius: 12, padding: "1.25rem" }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#0A1628", marginBottom: "0.5rem" }}>Year-on-year comparison</div>
        ...
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
          {[{ key: "monthly", label: "By month" }, { key: "area", label: "By area" }].map(t => (
            <button key={t.key} onClick={() => setView(t.key)} style={{
              ...
              background: view === t.key ? "#EDF4FC" : "#fff",
              color: view === t.key ? "#185FA5" : "#7A8499",
            }}>{t.label}</button>
          ))}
          ...
          {[{ key: "count", label: "Count" }, { key: "value", label: "Value" }].map(t => (
            <button key={t.key} onClick={() => setMetric(t.key)} style={{
              ...
              background: metric === t.key ? "#E1F5EE" : "#fff",
              color: metric === t.key ? "#0F6E56" : "#7A8499",
            }}>{t.label}</button>
          ))}
        </div>
      </div>
          ...
          {areaData.slice(0, 6).map(a => {
            ...
            return (
              <div key={a.area} style={{
                ...
                <span style={{ color: "#0A1628", fontWeight: 500 }}>{a.area}</span>
                {" "}
                <span style={{ color: up ? "#0F6E56" : "#A32D2D", fontWeight: 600 }}>
      ...
      {view === "monthly" ? (
        <ResponsiveContainer width="100%" height={chartHeight}>
          <BarChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey={currentYear} fill="#185FA5" radius={[3, 3, 0, 0]} maxBarSize={24} name={String(currentYear)} />
          </BarChart>
        </ResponsiveContainer>
      ) : (
        <ResponsiveContainer width="100%" height={chartHeight}>
          <BarChart data={chartData} layout="vertical" margin={{ left: 0, right: 8, top: 0, bottom: 0 }}>
            ...
            <Bar dataKey={currentYear} fill="#185FA5" radius={[0, 3, 3, 0]} maxBarSize={12} name={String(currentYear)} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
