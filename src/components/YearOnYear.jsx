      ...
      {payload.map(p => {
        ...
        return (
          <div key={p.name} style={{ color: p.color, marginBottom: 3 }}>
            {p.name}: <span style={{ color: "var(--surface)", fontWeight: 600 }}>{val}</span>
          </div>
        );
      })}
  ...
  if (years.length < 2) {
    return (
      <div style={{ background: "var(--surface)", border: "1px solid #E8ECF2", borderRadius: 12, padding: "1.25rem" }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginBottom: "0.5rem" }}>Year-on-year comparison</div>
          ...
          {areaData.slice(0, 6).map(a => {
            ...
            return (
              <div key={a.area} style={{
                ...
                <span style={{ color: "var(--text-primary)", fontWeight: 500 }}>{a.area}</span>
                {" "}
