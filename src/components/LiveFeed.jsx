...
const TYPE_COLORS = {
  Sale:     { bg:"#EDF4FC", color:"#185FA5" },
  Mortgage: { bg:"#FEF3E2", color:"#BA7517" },
  Gift:     { bg:"#E1F5EE", color:"#0F6E56" },
};
...
function LiveBadge() {
  ...
  return (
    <span style={{ display:"inline-flex", alignItems:"center", gap:5 }}>
      ...
      <span style={{ fontSize:11, fontWeight:600, color:"#1D9E75" }}>LIVE</span>
    </span>
  );
}
    ...
    <div style={{ padding:"1.25rem" }}>
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.25rem" }}>
        <div>
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            <div style={{ fontSize:18,fontWeight:700,color:"#0A1628" }}>Live Transactions</div>
            <LiveBadge />
          </div>
        ...
        <button onClick={load} style={{
          ...
          background:"#F8FAFC",border:"1px solid #E8ECF2",cursor:"pointer",color:"#7A8499"
        }}>↻ Refresh</button>
      </div>
      ...
      {stats && (
        <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:"1.25rem" }}>
          {[
            ["Today's deals",  fmtNum(stats.total),        "#0A1628"],
            ["Total value",    fmtAED(stats.totalVal,true), "#185FA5"],
            ["Sales",          fmtNum(stats.sales),         "#1D9E75"],
            ["Avg deal",       fmtAED(stats.avgVal,true),   "#0A1628"],
      ...
      {txns.length === 0 ? (
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:0 }}>
          {txns.map((r, i) => {
            ...
            const style = TYPE_COLORS[type] || { bg:"#F8FAFC", color:"#7A8499" };
