...
const FEATURES = [
  { icon:'📊', title:'354K+ Live Transactions', desc:'Complete DLD transaction history from 2020 to today. Every sale, mortgage and transfer in Dubai.' },
...
export default function LandingPage({ onLogin, onInvestorLogin, onBrokerLogin }) {
  ...
  const PlanCard = ({ plan, accentTone, audienceLabel }) => (
  ...
  return (
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", background: '#060E1A', color: '#F1F5F9', overflowX: 'hidden' }}>
      ...
      <section id="home" className="grid-bg" style={{ padding: '130px clamp(20px, 5vw, 80px) 80px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ fontSize: 18, color: '#94A3B8', maxWidth: 560, lineHeight: 1.65, marginBottom: 40 }}>
        ...
        {/* Stats */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', maxWidth: 660, width: '100%', background: 'rgba(13,25,41,0.85)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden', backdropFilter: 'blur(20px)' }}>
        ...
        <div style={{ marginTop: 14, fontSize: 12, color: '#475569', fontWeight: 500, letterSpacing: '0.02em' }}>Source: Dubai Land Department · Updated daily</div>
      </section>
      ...
      <section id="features" style={{ padding: '80px clamp(20px, 5vw, 80px)' }}>
        <div style={S.section}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 14 }}>
            {FEATURES.map((f, i) => (
              <div key={i} className="lp-card" style={{ background: '#0D1929', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: 24 }}>
                <div style={{ fontSize: 28, marginBottom: 14 }}>{f.icon}</div>
                <div style={{ fontSize: 15, fontWeight: 600, color: '#F1F5F9', marginBottom: 8, letterSpacing: '-0.005em' }}>{f.title}</div>
