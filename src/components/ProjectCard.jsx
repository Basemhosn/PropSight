...
export default function ProjectCard({ project, data, onClick, index }) {
  ...
  return (
    <div onClick={onClick} style={{
      background:"var(--surface)", border:"1px solid #E8ECF2", borderRadius:12,
    ...
    >
      <div style={{ height:160, overflow:"hidden", position:"relative", flexShrink:0 }}>
        ...
        <div style={{
          ...
          color:"var(--surface)", fontSize:10, fontWeight:600, padding:"3px 8px", borderRadius:20,
        }}>
      ...
      <div style={{ padding:"0.875rem", flex:1 }}>
        <div style={{
          fontSize:13, fontWeight:600, color:"var(--text-primary)", marginBottom:3, lineHeight:1.3,
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
          {[
            ["Transactions", fmtNum(kpis.count), "var(--text-primary)"],
            ["Total value",  fmtAED(kpis.total,true), "#38BDF8"],
            ["Avg deal",     fmtAED(kpis.avg,true), "var(--text-primary)"],
            ["Price/m²",     kpis.ppsqm>0 ? fmtAED(kpis.ppsqm,true) : "—", "#22C55E"],
          ].map(([label,value,color]) => (
            <div key={label} style={{ background:"var(--surface)", borderRadius:6, padding:"6px 8px" }}>
