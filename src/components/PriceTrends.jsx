...
const COLORS = ["#185FA5","#1D9E75","#D85A30","#BA7517","#993556","#534AB7","#3b6d11","#d4537e"];
    ...
    <div style={{ background:"#0A1628", color:"#fff", borderRadius:8, padding:"10px 14px", fontSize:12, minWidth:200 }}>
      ...
      {payload.map((p,i) => (
        <div key={i} style={{ color:p.color, marginBottom:3 }}>
          {p.name}: <span style={{ color:"#fff", fontWeight:600 }}>{fmtAED(p.value, true)}/m²</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ background:"#fff", border:"1px solid #E8ECF2", borderRadius:12, padding:"1.25rem" }}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:"1rem", flexWrap:"wrap", gap:8 }}>
        <div>
          <div style={{ fontSize:13, fontWeight:600, color:"#0A1628" }}>Price per m² trends</div>
        ...
        <div style={{ display:"flex", gap:4 }}>
          {[{k:"market",l:"Market"},{k:"area",l:"By area"}].map(t => (
            <button key={t.k} onClick={() => setMode(t.k)} style={{
              ...
              background: mode===t.k ? "#EDF4FC" : "#fff",
              color: mode===t.k ? "#185FA5" : "#7A8499",
            }}>{t.l}</button>
          ))}
        </div>
      </div>
          ...
          {topAreas.map((a,i) => {
            ...
            return (
              <button key={a} onClick={() => toggleArea(a)} style={{
                ...
                background: sel ? COLORS[i%COLORS.length]+"18" : "#fff",
      ...
      <ResponsiveContainer width="100%" height={280}>
        {mode === "market" ? (
          <LineChart data={marketData} margin={{ top:4, right:16, left:0, bottom:0 }}>
            ...
            <Line type="monotone" dataKey="ppsqm" stroke="#185FA5" strokeWidth={2.5}
              dot={{ r:3, fill:"#185FA5" }} activeDot={{ r:5 }} name="Market avg" />
          </LineChart>
        ) : (
