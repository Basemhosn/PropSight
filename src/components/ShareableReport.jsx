        ...
        {selectedArea && areaStats ? (
          <div style={{background:'var(--surface)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:24}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
              {[['Transactions',fmtNum(kpis.count||0),'var(--text-primary)'],['Avg Deal',fmtAED(kpis.avg||0,true),'#38BDF8'],['Price/sqft','AED '+fmtNum(Math.round((kpis.ppsqm||0)/10.764)),'var(--text-primary)'],['YoY Growth',`${yoy>0?'+':''}${yoy}%`,yoy>0?'#22C55E':'#F87171']].map(([l,v,c],i)=>(
            ...
            <div style={{marginBottom:16}}>
              <div style={{fontSize:12,fontWeight:600,color:'var(--text-primary)',marginBottom:12}}>Price per sqft — Historical Trend</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              {[['Off-Plan',fmtNum(kpis.offPlan||0),'#38BDF8',Math.round((kpis.offPlan||0)/(kpis.count||1)*100)+'%'],['Ready',fmtNum(kpis.ready||0),'#22C55E',Math.round((kpis.ready||0)/(kpis.count||1)*100)+'%']].map(([l,v,c,pct],i)=>(
                <div key={i} style={{background:'rgba(59,130,246,0.04)',border:'1px solid rgba(59,130,246,0.08)',borderRadius:10,padding:14,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div><div style={{fontSize:11,color:'var(--text-muted)',marginBottom:4}}>{l}</div><div style={{fontSize:16,fontWeight:700,color:c}}>{v}</div></div>
