...
export default function ProjectDetail({ project, data, onClose }) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div style={{ position:"fixed",right:0,top:0,bottom:0,width:480,background:"#fff",
        ...
        <div style={{ height:200,flexShrink:0,position:"relative",overflow:"hidden" }}>
          {!imgError && data.image ? (
            ...
            <div style={{ width:"100%",height:"100%",background:"linear-gradient(135deg,#0A1628 0%,#185FA5 100%)",
              ...
              <div style={{ fontSize:18,fontWeight:700,color:"#fff",textAlign:"center",lineHeight:1.4 }}>{project}</div>
            </div>
          )}
          {/* Gradient overlay */}
          ...
          <div style={{ position:"absolute",bottom:12,left:16,right:48 }}>
            <div style={{ fontSize:16,fontWeight:700,color:"#fff",lineHeight:1.3,marginBottom:3 }}>{project}</div>
        ...
        <div style={{ display:"flex",borderBottom:"1px solid #E8ECF2",flexShrink:0 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={()=>setTab(t.id)} style={{
              ...
              color:tab===t.id?"#185FA5":"#7A8499",background:"none",border:"none",cursor:"pointer",
        ...
        <div style={{ padding:"1.25rem",flex:1 }}>
          ...
          {tab === "overview" && (
            <>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:"1.25rem" }}>
                ...
                <KPI label="Total value"    value={fmtAED(kpis.total,true)}   color="#185FA5" />
                <KPI label="Avg deal"       value={fmtAED(kpis.avg,true)}     />
                <KPI label="Avg size"       value={kpis.avgSize>0?kpis.avgSize+"m²":"—"} />
                <KPI label="Price/m²"       value={kpis.ppsqm>0?fmtAED(kpis.ppsqm,true):"—"} color="#1D9E75" />
                <KPI label="Off-plan"       value={offPct+"%"}                color="#185FA5" />
              </div>

              {/* Price range */}
              <div style={{ background:"#F8FAFC",borderRadius:8,padding:"0.875rem",marginBottom:"1.25rem" }}>
                ...
                <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize:14,fontWeight:600,color:"#1D9E75" }}>{fmtAED(kpis.minPrice,true)}</div>
                  </div>
                  ...
                  <div style={{ textAlign:"right" }}>
                    ...
                    <div style={{ fontSize:14,fontWeight:600,color:"#185FA5" }}>{fmtAED(kpis.maxPrice,true)}</div>
                  </div>
                </div>
              </div>
              ...
              {/* Monthly trend */}
              <div style={{ fontSize:12,fontWeight:600,color:"#0A1628",marginBottom:8 }}>Monthly volume</div>
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={data.monthly} margin={{top:0,right:0,left:0,bottom:0}}>
                  ...
                  <YAxis hide />
                  <Tooltip formatter={(v)=>[fmtNum(v),'Txns']} contentStyle={{background:"#0A1628",border:"none",borderRadius:8,color:"#fff",fontSize:11}} />
          ...
          {tab === "price" && (
            <>
              <div style={{ fontSize:12,fontWeight:600,color:"#0A1628",marginBottom:8 }}>Price per m² by year</div>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={data.priceTrend} margin={{top:4,right:16,left:0,bottom:0}}>
                  ...
                  <Tooltip formatter={(v)=>[fmtAED(v,true)+'/m²','Price']} contentStyle={{background:"#0A1628",border:"none",borderRadius:8,color:"#fff",fontSize:11}} />
                  <Line type="monotone" dataKey="ppsqm" stroke="#1D9E75" strokeWidth={2.5} dot={{r:4,fill:"#1D9E75"}} />
                </LineChart>
              </ResponsiveContainer>

              <div style={{ marginTop:"1.25rem",fontSize:12,fontWeight:600,color:"#0A1628",marginBottom:8 }}>Transaction type split</div>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  ...
                  <Tooltip formatter={(v)=>[fmtNum(v),'Deals']} contentStyle={{background:"#0A1628",border:"none",borderRadius:8,color:"#fff",fontSize:11}} />
                </PieChart>
              </ResponsiveContainer>

              <div style={{ marginTop:"1.25rem",fontSize:12,fontWeight:600,color:"#0A1628",marginBottom:8 }}>Price range summary</div>
              {[
                ...
                <div key={label} style={{ display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #F4F6FA" }}>
                  ...
                  <div style={{ fontSize:12,color:"#0A1628",fontWeight:500 }}>{value}</div>
                </div>
              ))}
            </>
          )}
          ...
          {tab === "units" && (
            <>
              <div style={{ fontSize:12,fontWeight:600,color:"#0A1628",marginBottom:"1rem" }}>Unit breakdown by bedroom type</div>
              {(data.rooms||[]).map((r, i) => {
                ...
                return (
                  <div key={r.rooms} style={{ marginBottom:16 }}>
                    <div style={{ display:"flex",justifyContent:"space-between",marginBottom:4 }}>
                      <div style={{ fontSize:13,fontWeight:500,color:"#0A1628" }}>{r.rooms}</div>
                      <div style={{ fontSize:11,color:"#9AA0AE" }}>{fmtNum(r.count)} units</div>
                    </div>
                    <div style={{ height:8,background:"#F4F6FA",borderRadius:4,overflow:"hidden",marginBottom:4 }}>
                    ...
                    <div style={{ display:"flex",gap:16,fontSize:11,color:"#9AA0AE" }}>
                      <span>Avg price: <span style={{ color:"#0A1628",fontWeight:500 }}>{fmtAED(r.avg,true)}</span></span>
                      {r.avgSize > 0 && <span>Avg size: <span style={{ color:"#0A1628",fontWeight:500 }}>{r.avgSize}m²</span></span>}
                    </div>
                  </div>
                );
              })}
              ...
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1rem",gap:8 }}>
                ...
                <input value={txnSearch} onChange={e=>setTxnSearch(e.target.value)}
                  ...
                  style={{ fontSize:12,padding:"6px 10px",borderRadius:8,border:"1px solid #E8ECF2",
                    width:180,color:"#0A1628",background:"#F8FAFC",outline:"none" }} />
              </div>
              ...
              {filteredTxns.map((r,i) => (
                <div key={i} style={{ padding:"10px 0",borderBottom:"1px solid #F4F6FA" }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8 }}>
                    <div>
                      <div style={{ fontSize:13,fontWeight:600,color:"#0A1628" }}>{fmtAED(r.amount,true)}</div>
