  ...
  if (!isLite) return (
    <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:'#060E1A',fontFamily:'system-ui',padding:24}}>
      <div style={{textAlign:'center',maxWidth:380}}>
        ...
        <div style={{fontSize:22,fontWeight:700,color:'#F1F5F9',marginBottom:8}}>Price Alerts</div>
        <div style={{fontSize:14,color:'#64748B',marginBottom:24,lineHeight:1.6}}>Get notified by email when Dubai area prices move by your chosen threshold.</div>
  ...
  return (
    <div style={{flex:1,overflowY:'auto',background:'#060E1A',fontFamily:'system-ui',padding:'24px 28px'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
        <div>
          <h1 style={{margin:0,fontSize:22,fontWeight:700,color:'#F1F5F9',marginBottom:4}}>Price Alerts</h1>
      ...
      : alerts.length===0 ? (
        <div style={{textAlign:'center',padding:60}}>
          <div style={{fontSize:48,marginBottom:16}}>🔔</div>
          <div style={{fontSize:18,fontWeight:600,color:'#F1F5F9',marginBottom:8}}>No alerts yet</div>
          ...
          {alerts.map((alert,i)=>{
            ...
            return (
              <div key={alert.id} style={{padding:'18px 20px',borderBottom:i<alerts.length-1?'1px solid rgba(255,255,255,0.04)':'none',display:'flex',alignItems:'center',gap:16}}>
                <div style={{width:44,height:44,borderRadius:10,background:alert.active?'rgba(59,130,246,0.1)':'rgba(100,116,139,0.1)',border:`1px solid ${alert.active?'rgba(59,130,246,0.2)':'rgba(100,116,139,0.15)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>🔔</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,fontWeight:700,color:'#F1F5F9',marginBottom:2}}>{alert.building_name}</div>
                ...
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <button onClick={()=>toggleAlert(alert.id,alert.active)} style={{padding:'6px 12px',borderRadius:20,border:'none',cursor:'pointer',fontSize:11,fontWeight:600,fontFamily:'system-ui',background:alert.active?'rgba(34,197,94,0.15)':'rgba(100,116,139,0.1)',color:alert.active?'#22C55E':'#64748B'}}>{alert.active?'Active':'Paused'}</button>
      ...
      {showAdd && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
          <div style={{background:'#0D1929',border:'1px solid rgba(59,130,246,0.2)',borderRadius:16,padding:28,width:'100%',maxWidth:440,fontFamily:'system-ui'}}>
            <div style={{fontSize:18,fontWeight:700,color:'#F1F5F9',marginBottom:20}}>New Price Alert</div>
